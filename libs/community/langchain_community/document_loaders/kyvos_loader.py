...
class KyvosLoader(BaseLoader):
    ...
    def __init__(self, configuration_parameters:dict, username: Optional[str]=None, password:Optional[str]=None, query: str=None,jwt_token:str = None, schema:str = '.metadata, .rows[]'):
        ...
        if self.jwt_token==None:
            ...
            if self.username == None:
                raise ValueError(f"Got Null value for Kyvos_username. Either pass the Kyvos_username or set the value in enviornment variable by 'KYVOS_USERNAME'")
            self.password = os.getenv('KYVOS_PASSWORD') or password
            if self.password == None:
                raise ValueError(f"Got Null value for Kyvos_password. Either pass the Kyvos_password or set the value in enviornment variable by 'KYVOS_PASSWORD'")
        else:
            self.username == None
            self.password == None
    ...
    def get_headers(self) -> str:
        ...
        if self.__dict__.get("login_url",None)!=None:
            try:
                ...
                response = requests.post(url=self.login_url, headers=self.conn_headers, data=self.conn_body,)
    ...
    def lazy_load(self):
        ...
        try:
            ...
            try:
                with requests.post(self.query_url, stream=True, data=self.payload, headers=headers) as response:
                    response.raise_for_status()
            ...
            if self.zipped == 'true':
                with zipfile.ZipFile(self.file_path) as z:
                    ...
                    self.file_path = [file for file in os.listdir(self.temp_dir.name) if file.endswith('.csv') or file.endswith('.json')][0]
